...
class FeatureConverter(
    ...
    init {
        tokenizer = BertTokenizer(inputDic, doLowerCase)
    ...
    fun convert(query: String?, context: String): Feature {
        ...
        val origTokens = listOf(*context.trim { it <= ' ' }.split("\\s+".toRegex())
        ...
        tokens.add("[SEP]")
        segmentIds.add(1)
        val inputIds: MutableList<Int?> = tokenizer.convertTokensToIds(tokens) as MutableList<Int?>
